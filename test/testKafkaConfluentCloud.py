...
import unittest
import os
from util.newAbstractMessageBroker import AbstractMessageBrokerHandler, json_deserializer, json_serializer
...
class TestMessageBrokerHandler(AbstractMessageBrokerHandler):
    ...
    def __init__(self, bootstrap_servers, inbound_event, outbound_event, service ='kafka-test'):
        super().__init__( 
                bootstrap_servers = bootstrap_servers,
                default_inbound_event = inbound_event, 
                default_outbound_event = outbound_event,
                service = service,
...
INBOUND_TOPIC1 = "checkmated.t1.TestMessageBrokerHandler.test-input"
OUTBOUND_EVENT1 = "test-output"
OUTBOUND_TOPIC1 = "checkmated.t1.kafka-test.test-output"
TEST_TOPIC = "checkmated.t1.TestMessageBroker.test-input1"
SECURITY_PROTOCOL ="SASL_SSL"
KAFKA_SASL_MECHANISM = "PLAIN"
KAFKA_SASL_USERNAME='UD7DUM6NGGSWAQTY' 
KAFKA_SASL_PASSWORD='wfjVn2/23+U8RqYhurgQSgLvRJZYMp1QXrKDV/GlshhUCCMPJvdj8lcTYKizowKl'

KAFKA_BOOTSTRAP_SERVERS = os.getenv("KAFKA_HOST", "pkc-56d1g.eastus.azure.confluent.cloud:9092")
CLIENT_ID = "test"
...
class TestMessageBroker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        admin_conf.update(self.sasl_conf)

        self.admin_client = AdminClient(admin_conf)

        new_topics = [NewTopic(topic, num_partitions=3, replication_factor=3) for topic in [INBOUND_TOPIC1, OUTBOUND_TOPIC1, TEST_TOPIC]]
    ...
    def test_messageBrokerHandler(self):
        ...
        producer_conf.update(self.sasl_conf)
        producer = Producer(producer_conf)
        
        consumer_conf.update(self.sasl_conf)
        ...
        def listen_message1():
            ...
            messageHandler = TestMessageBrokerHandler(
                ...
                inbound_event = INBOUND_TOPIC1, 
                outbound_event = OUTBOUND_EVENT1)
        ...
        for i in range(messages):
            ...
            producer.produce(INBOUND_TOPIC1, key=str(uuid.uuid4()).encode('utf-8'), value=json_serializer(test_message))
        producer.flush()
        ...
        while len(msgs) < messages:
            try:
                ...
                if msg is None:
                    ...
                    payload = json.loads(bytes(json.loads(msg.value())['data']['value']))['data']
                    msgs.add(payload)
