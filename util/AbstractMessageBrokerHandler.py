...
def build_inbound_topic_list(service, events, environment=os.getenv('KAFKA_ENVIRONMENT','dev'), tenants = os.getenv("KAFKA_TENANTS", "t1,t2,t3,t4,t5")):
        topics = []
        # KAFKA_CATALOG_SERVICE
        tenants = tenants.replace(" ","").split(",")
        for t in tenants:
            for event in events:
                topics.append(f'{environment}.{t}.{service}.{event}')
        return topics
...
class AbstractMessageBrokerHandler(metaclass=ABCMeta):
    ...
    def __init__(self, 
                ...
                environment = os.getenv('KAFKA_PRODUCTION_ENVIRONMENT','dev'),
                use_cloud=True,
                group_id = 'checkmate', 
                key_prefix=None, 
                max_workers=16):
        ...
        """
        self.inbound_events = default_inbound_event if type(default_inbound_event) == list else [default_inbound_event]
        ...
        self.consumer.subscribe(self.inbound_events)
        
        logger.debug(f'Created KafkaConsumer listening to {self.inbound_events} at {bootstrap_servers}')
        ...
        logger.debug(f'Created KafkaProducer producing to outbound_topic = {self.environment}.t?.{self.service}.{self.default_outbound_event} using bootstrap {bootstrap_servers}')
    ...
    def _consume_message(self, message):
        ...
        if self.max_workers > 0:
            ...
            logger.debug(f'Message commited (running workers:{self.running_workers}).')
    ...
    def produce_output_message(self, payload, tenant, original_message=None,event=None):
        ...
        try:
            ...
            self.producer.flush()
            logger.debug(f'Message sent (key: {message_key}) to {outbound_topic}')
    ...
    def start(self):
        ...
        try:
            ...
            logger.info(f'CLOSING consumer {self.consumer} ')
            self.consumer.close()
            return 1 
